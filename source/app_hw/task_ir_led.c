 ...
 ******************************************************************************/
static void task_ir_led(void *param);
...
QueueHandle_t q_ir_led_req;
QueueHandle_t q_ir_led_resp;
...
static void task_ir_led(void *param)
{
    ...
    for (;;)
    {
        ...
        xQueueReceive(q_ir_led_req, &ir_led_pkt, portMAX_DELAY);
...
)
{
    ...
    ir_led_pkt.return_queue = q_ir_led_resp;
    xQueueSendToBack(q_ir_led_req, &ir_led_pkt, portMAX_DELAY);

    // Wait for pairing to complete
    xQueueReceive(q_ir_led_resp, &ir_led_pkt, portMAX_DELAY);
...
void task_ir_led_init(void)
{
    ...
    q_ir_led_req  = xQueueCreate(1, sizeof(ir_led_packet_t));
    q_ir_led_resp = xQueueCreate(1, sizeof(ir_led_packet_t));

    // Register the CLI commands
    FreeRTOS_CLIRegisterCommand(&xIrLedSetState);

    // Create the task that will control the IR LED
    xTaskCreate(
        task_ir_led,
        "Task_IR_LED",
