...
QueueHandle_t q_audio_req;
QueueHandle_t q_audio_resp;
...
static void task_audio(void *param)
{
    ...
    for (;;)
    {
        ...
        xQueueReceive(q_audio_req, &audio_pkt, portMAX_DELAY);
        ...
        switch(audio_pkt.cmd) {
            case AUDIO_PLAY_TONE:
                ...
                {
                    transformedSineWaveLUT[i] = ((audio_pkt.amplitude * rawSineWaveLUT[i]) / amp_range);
                }
                ...
                // Update frequency
                uint32_t divider = (1000000 / audio_pkt.frequency) - 1;
            ...
            case AUDIO_PLAY_TUNE:
                ...
                // Set divider to so that the tune is replayed at the sample rate used to process the original audio file ((max_clock_rate_hz / sample_rate_hz) - 1)
                (void)Cy_SysClk_PeriphDisableDivider(peri_0_div_24_5_0_HW, peri_0_div_24_5_0_NUM);
...
)
{
    ...
    audio_pkt.return_queue = q_audio_resp;
    xQueueSendToBack(q_audio_req, &audio_pkt, portMAX_DELAY);

    // Wait for task to complete
    xQueueReceive(q_audio_resp, &audio_pkt, portMAX_DELAY);
...
void task_audio_init(void)
{
    ...
    q_audio_req  = xQueueCreate(1, sizeof(audio_packet_t));
    q_audio_resp = xQueueCreate(1, sizeof(audio_packet_t));

    // Register the CLI commands
    FreeRTOS_CLIRegisterCommand(&xAudioPlayTone);
    FreeRTOS_CLIRegisterCommand(&xAudioPlayTune);

    // Create the task that will control audio
    xTaskCreate(
        task_audio,
        "Task_Audio",
